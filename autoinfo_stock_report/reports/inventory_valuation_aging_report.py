...
class InventoryValuationAgingReport(models.TransientModel):
    _inherit = 'inventory.valuation.aging.report'

    p_department_id = fields.Many2one('hr.department', string='Department')

    def get_context(self):
        context = super(InventoryValuationAgingReport, self).get_context()
        context.update({'default_p_department_id': self.p_department_id.id})
        return context

    def get_parameters(self):
        res = super(InventoryValuationAgingReport, self).get_parameters()
